...
/*		request.setCharacterEncoding("utf-8");
		...
		RequestDispatcher le = request.getRequestDispatcher("LoginError.jsp");
		RequestDispatcher ls = request.getRequestDispatcher("LoginSuccess.jsp");
		
		if(tempUser == null) {
			System.out.println("none id");
			//response.sendRedirect("LoginError");  //id가 존재하지 않을때.
			le.forward(request, response);// 이때 페이지 이동한다...
		}
		else {
			if(tempUser.getPassword().equals(pass)) {
				//response.sendRedirect("LoginSuccess?i="+id+"&p="+pass);  //객체가 넘겨지는지 확인해봐야겠음.
				ls.forward(request, response);// 이때 페이지 이동한다...
			}
			else {
